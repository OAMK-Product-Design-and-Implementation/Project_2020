...
class PictureService {
  static PictureService _instance = null;
  ...
  static Future<PictureService> getInstance() async{
    ...
    if(_externalStorageDirectories == null){
      _externalStorageDirectories = await getExternalStorageDirectories(type: StorageDirectory.pictures);
    }
    ...
    _picturesDirectory = Directory("/storage/emulated/0/SecurityControl/Pictures");
    return _instance;
  }
  ...
  Future<List<List>> fetchPhotos(http.Client client) async {
    ...
    if(_fileList.length == 0){
      _query = "http://" + _serverAddress + "/api/imagesbytime/get/2000-01-01 00:00:00";
    }
    else {
      for (var f in _fileList) {
        if (f is File) {
          print(_debugTag + (f as File).path.toString());
          ...
          if (p.extension(f.path) == ".jpg") {
            ...
            _pictureList.add([f, DateTime.parse(filename.substring(filename.length - 15, filename.length))]);


            DateTime dateOfFile = DateTime.parse(filename.substring(filename.length - 15, filename.length));
      ...
      print(_debugTag + "Latest picture date: " + _latestPictureDate.toString());
      _query = "http://" + _serverAddress + "/api/imagesbytime/get/" + _latestPictureDate.toString();
    }
    ...
    if(!(response == null)){
      try{
        ...
        print(_debugTag + "ERROR: Could not process images from server response: " + err.toString());
      }
    }
    ...
    if(imageJson is List){
      for(var itemList in imageJson){
        ...
        File newImage = await File(_picturesDirectory.path + "/" + itemList[0]
            + "_" + DateTime.now().millisecondsSinceEpoch.toString()
            + "_" + _dateFormatWrite.format(DateTime.parse(itemList[3])).toString() + ".jpg").create();

        if(bytes.length != 0){
          _pictureList.add([await newImage.writeAsBytes(bytes), DateTime.parse(itemList[3])]);
        }
      }
    }

    // TODO: Order photos by date???
